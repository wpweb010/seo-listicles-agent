...
def register_routes(app, agent, database):
    ...
    async def list_api_keys():
        ...
        # Attach usage info to each
        usage_data = database.get_api_usage()
        for k in keys:
            k["usage"] = usage_data.get(k["provider"], {})
        return {"keys": keys}
    ...
    async def save_api_key(req: ApiKeyRequest):
        ...
        database.upsert_api_key(provider, req.api_key.strip(), req.extra_data or {})
    ...
    async def test_api_key_endpoint(provider: str):
        provider = provider.lower()
        if provider not in KEY_TESTERS:
            raise HTTPException(400, f"Unknown provider: {provider}")
        key_row = database.get_api_key(provider)
        if not key_row:
            raise HTTPException(404, "API key not saved")
        result = KEY_TESTERS[provider](key_row["api_key"], key_row.get("extra_data"))
        database.set_api_key_status(provider, "ok" if result["ok"] else "error")
        return result
